...
export function getModelChannelState(model: Model, t: TFunction) {
  ...
  if (configured === 0) {
    if (model.name_rule !== 0) {
      return {
        label: t('No matching channels'),
        description: t('No configured channel models match this metadata rule.'),
      }
    }
    return {
  ...
  return {
    label: t('Available channels: {{count}}', { count: available }),
    description: t(
