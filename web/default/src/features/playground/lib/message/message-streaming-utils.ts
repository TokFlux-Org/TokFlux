...
export function applyChatCompletionChoice(
  ...
  const messageContent = contentToTextWithImages(
  ...
  return completeAssistantTiming({
    ...finalizeMessage(
      updateCurrentVersionContent(message, messageContent),
...
function contentToTextWithImages(
  content: ChatCompletionChoice['message']['content'] | undefined,
  images: unknown
): string {
  const text = content ? getTextContent(content) : ''
  const imageMarkdown = normalizeImageMarkdown(images)
